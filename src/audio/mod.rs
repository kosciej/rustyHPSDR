...
impl Audio {
    ...
    pub fn open_input(&mut self) -> Result<(), Error> {
        ...
        let pcm = PCM::new(&self.input_device, Direction::Capture, false)?;
        {
            ...
            hwp.set_channels(2)
                .expect("create_input failed to set channels");
    ...
    pub fn read_input(&mut self) -> Vec<i16> {
        ...
        let io = match self
            .input
            .as_ref()
            .expect("failed to get self.input")
            .io_i16()
        {
            Ok(i) => i,
            Err(e) => {
    ...
    pub fn write_output(&mut self, buffer: &Vec<i16>) -> Result<(), Error> {
        match self
            .output
            .as_ref()
            .expect("Could not access output to get delay")
            .delay()
        {
            Ok(delay) => {
                let mut trim = 0;
                ...
                let io = self
                    .output
                    .as_ref()
                    .expect("write_output: failed to get io")
                    .io_i16()
                    .expect("write_output: failed to get io_i16");
                if trim > 0 {
                        ...
                        match io.writei(&trimmed_buffer) {
                            ...
                            Err(e) => {
                                ...
                                match self
                                    .output
                                    .as_ref()
                                    .expect("output failed for prepare")
                                    .prepare()
                                {
                                    Ok(()) => {}
                                    Err(e) => {
