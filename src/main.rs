...
use gtk::prelude::*;
use gtk::{
    Adjustment, Application, ApplicationWindow, Builder, Button, DrawingArea, DropDown, Frame,
    Grid, Label, ToggleButton,
};
use gtk::{
    EventController, EventControllerMotion, EventControllerScroll, EventControllerScrollFlags,
    GestureClick,
};
...
fn build_ui(app: &Application) {
    ...
    let c_string = match CString::new(os_string.into_vec()) {
        ...
        Err(e) => {
            eprintln!(
                "Error converting path to C string (contains null bytes?): {}",
                e
            );
            return;
        }
    ...
    let rc_app_widgets_clone = rc_app_widgets.clone();
    let app_widgets = rc_app_widgets_clone.borrow();
    let discovery_dialog = create_discovery_dialog(
        &app_widgets.main_window.clone(),
        discovery_data_clone,
        selected_index_for_discovery_dialog,
    );
        ...
        match index {
            Some(i) => {
                if i >= 0 {
                    ...
                    let radio_mutex = RadioMutex::new(Arc::new(Mutex::new(Radio::load(
                        device,
                        app_widgets.spectrum_display.width(),
                    ))));
                    ...
                    // setup the ui state
                    {
                        ...
                        if r.receiver[0].ctun {
                            let formatted_value =
                                format_u32_with_separators(r.receiver[0].ctun_frequency as u32);
                            app_widgets.vfo_a_frequency.set_label(&formatted_value);
                        } else {
                            let formatted_value =
                                format_u32_with_separators(r.receiver[0].frequency as u32);
                            app_widgets.vfo_a_frequency.set_label(&formatted_value);
                        }

                        if r.receiver[1].ctun {
                            let formatted_value =
                                format_u32_with_separators(r.receiver[1].ctun_frequency as u32);
                            app_widgets.vfo_b_frequency.set_label(&formatted_value);
                        } else {
                            let formatted_value =
                                format_u32_with_separators(r.receiver[1].frequency as u32);
                            app_widgets.vfo_b_frequency.set_label(&formatted_value);
                        }
                        ...
                        app_widgets
                            .afgain_adjustment
                            .set_value((r.receiver[rx].afgain * 100.0).into());
                        app_widgets
                            .agc_dropdown
                            .set_selected(r.receiver[rx].agc as u32);
                        app_widgets
                            .agcgain_adjustment
                            .set_value(r.receiver[rx].agcgain.into());
                        app_widgets
                            .micgain_adjustment
                            .set_value(r.transmitter.micgain.into());
                        app_widgets
                            .micgain_adjustment
                            .set_value(r.transmitter.micgain.into());
                        app_widgets
                            .drive_adjustment
                            .set_value(r.transmitter.drive.into());
                        app_widgets
                            .cwpitch_adjustment
                            .set_value(r.receiver[rx].cw_pitch.into());

                        let rc_spectrum_clone2 = rc_spectrum_clone.clone();
                        let mut spectrum = rc_spectrum_clone2.borrow_mut();
                        spectrum.resize(
                            app_widgets.spectrum_display.width(),
                            app_widgets.spectrum_display.height(),
                        );
                        ...
                        let mut spectrum = rc_spectrum_2_clone2.borrow_mut();
                        spectrum.resize(
                            app_widgets.spectrum_2_display.width(),
                            app_widgets.spectrum_2_display.height(),
                        );
                        ...
                        let mut waterfall = rc_waterfall_clone2.borrow_mut();
                        waterfall.resize(
                            app_widgets.waterfall_display.width(),
                            app_widgets.waterfall_display.height(),
                        );
                        r.receiver[0].spectrum_width = app_widgets.spectrum_display.width();
                        ...
                        let mut waterfall = rc_waterfall_2_clone2.borrow_mut();
                        waterfall.resize(
                            app_widgets.waterfall_2_display.width(),
                            app_widgets.waterfall_2_display.height(),
                        );
                        r.receiver[1].spectrum_width = app_widgets.spectrum_2_display.width();

                        app_widgets
                            .step_dropdown
                            .set_selected(r.receiver[rx].step_index as u32);
                        app_widgets
                            .zoom_adjustment
                            .set_value(r.receiver[rx].zoom.into());
                        app_widgets
                            .pan_adjustment
                            .set_value(r.receiver[rx].pan.into());

                        let style_context = app_widgets.nr_button.style_context();
                        style_context.add_class("toggle");
                        app_widgets
                            .nr_button
                            .set_active(r.receiver[rx].nr | r.receiver[rx].nr2);
                        if r.receiver[rx].nr {
                        ...
                        app_widgets
                            .nb_button
                            .set_active(r.receiver[rx].nb | r.receiver[rx].nb2);
                        if r.receiver[rx].nb {
                    ...
                    app_widgets.configure_button.connect_clicked(move |_| {
                        ...
                        let configure_dialog =
                            create_configure_dialog(&app_widgets.main_window, &radio_mutex_clone);
                    ...
                    app_widgets
                        .vfo_a_frequency
                        .add_controller(scroll_controller_a);

                    let scroll_controller_b =
                        EventControllerScroll::new(EventControllerScrollFlags::VERTICAL);
                    let radio_mutex_clone = radio_mutex.clone();
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    ...
                    app_widgets
                        .vfo_b_frequency
                        .add_controller(scroll_controller_b);
                    ...
                    app_widgets.ctun_button.connect_clicked(move |button| {
                        ...
                        if r.receiver[rx].ctun {
                            ...
                            let formatted_value =
                                format_u32_with_separators(r.receiver[rx].frequency as u32);
                            if rx == 0 {
                                app_widgets.vfo_a_frequency.set_label(&formatted_value);
                            } else {
                                app_widgets.vfo_b_frequency.set_label(&formatted_value);
                    ...
                    app_widgets.rx2_button.connect_clicked(move |button| {
                        ...
                        if update {
                            update_ui(
                                &radio_mutex_clone.clone(),
                                &rc_app_widgets_clone_clone.clone(),
                            );
                        }
                    });
                    ...
                    app_widgets
                        .step_dropdown
                        .connect_selected_notify(move |step_dropdown| {
                            let mut r = radio_mutex_clone.radio.lock().unwrap();
                    ...
                    spectrum_click_gesture_clone.connect_pressed(
                        move |gesture, controller, x, _y| {
                            ...
                            if gesture.current_button() == 2 {
                                ...
                                *press_state.borrow_mut() = true;
                                ...
                                let notch = Notch::new(
                                    rx as i32,
                                    r.receiver[rx].frequency as f64,
                                    500.0,
                                    1,
                                );
                    ...
                    let press_state = middle_button_pressed.clone();
                    spectrum_click_gesture_clone.connect_released(
                        move |gesture, controller, x, _y| {
                            if gesture.current_button() == 2 {
                                // middle button
                    ...
                    app_widgets.spectrum_display.add_controller(
                        <GestureClick as Clone>::clone(&spectrum_click_gesture)
                            .upcast::<EventController>(),
                    );
                    ...
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    let press_state = middle_button_pressed.clone();
                    spectrum_2_click_gesture_clone.connect_pressed(
                        move |gesture, controller, x, _y| {
                            let da = gesture.widget().unwrap();
                            let width = da.allocated_width();
                            if gesture.current_button() == 2 {
                                // middle button
                                *press_state.borrow_mut() = true;
                            } else if !spectrum_waterfall_clicked(
                                &radio_mutex_clone,
                                &rc_app_widgets_clone_clone,
                                1,
                                x,
                    ...
                    let press_state = middle_button_pressed.clone();
                    spectrum_2_click_gesture_clone.connect_released(
                        move |gesture, controller, x, _y| {
                            if gesture.current_button() == 2 {
                                // middle button
                                *press_state.borrow_mut() = false;
                    ...
                    app_widgets.spectrum_2_display.add_controller(
                        <GestureClick as Clone>::clone(&spectrum_2_click_gesture)
                            .upcast::<EventController>(),
                    );
                    ...
                    app_widgets
                        .spectrum_display
                        .add_controller(motion_event_controller_spectrum.clone());
                    ...
                    motion_event_controller_spectrum.connect_motion(move |controller, x, y| {
                        ...
                        if x < 40.0 {
                            ...
                            if y < top.into() {
                                app_widgets
                                    .spectrum_display
                                    .set_cursor(cursor_uparrow.as_ref());
                            } else if y > bottom.into() {
                                app_widgets
                                    .spectrum_display
                                    .set_cursor(cursor_downarrow.as_ref());
                            } else {
                                app_widgets
                                    .spectrum_display
                                    .set_cursor(cursor_nsresize.as_ref());
                            }
                        } else {
                    ...
                    app_widgets
                        .spectrum_2_display
                        .add_controller(motion_event_controller_spectrum_2.clone());
                    let last_spectrum_x_clone = last_spectrum_x.clone();
                    let last_spectrum_y_clone = last_spectrum_y.clone();
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    ...
                    app_widgets
                        .spectrum_display
                        .add_controller(scroll_controller_spectrum.clone());
                    ...
                    app_widgets
                        .spectrum_2_display
                        .add_controller(scroll_controller_spectrum_2.clone());
                    ...
                    app_widgets
                        .waterfall_display
                        .add_controller(scroll_controller_waterfall.clone());
                    ...
                    app_widgets
                        .waterfall_2_display
                        .add_controller(scroll_controller_waterfall_2.clone());
                    ...
                    app_widgets.waterfall_display.add_controller(
                        <GestureClick as Clone>::clone(&waterfall_click_gesture)
                            .upcast::<EventController>(),
                    );
                    ...
                    app_widgets.waterfall_2_display.add_controller(
                        <GestureClick as Clone>::clone(&waterfall_2_click_gesture)
                            .upcast::<EventController>(),
                    );
                    ...
                    app_widgets
                        ...
                        .connect_value_changed(move |adjustment| {
                            ...
                            if adjustment.value() == 1.0 {
                                ...
                                let frequency_low = r.receiver[rx].frequency
                                    - (r.receiver[rx].sample_rate / 2) as f32;
                                let frequency_high = r.receiver[rx].frequency
                                    + (r.receiver[rx].sample_rate / 2) as f32;
                    ...
                    let radio_mutex_clone = radio_mutex.clone();
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    app_widgets.band_grid.set_callback(
                        move |index| {
                            let app_widgets = rc_app_widgets_clone_clone.borrow();
                            ...
                            if b != index {
                                ...
                                // get new band info
                                r.receiver[rx].band =
                                    Bands::from_usize(index).expect("invalid band index");
                                ...
                                if !r.receiver[rx].filters_manual {
                                    r.receiver[rx].filters =
                                        r.receiver[rx].band_info[index].filters;
                                }
                            }
                            app_widgets.filter_grid.update_filter_buttons(
                                r.receiver[rx].band_info[index].mode.to_usize(),
                            );
                            app_widgets.filter_grid.set_active_index(
                                r.receiver[rx].band_info[index].filter.to_usize(),
                            );
                            ...
                            if b != index {
                                ...
                                let (low, high) = app_widgets.filter_grid.get_filter_values(
                                    r.receiver[rx].band_info[index].mode.to_usize(),
                                    r.receiver[rx].band_info[index].filter.to_usize(),
                                );
                                app_widgets.filter_grid.set_active_values(low, high);
                    ...
                    let radio_mutex_clone = radio_mutex.clone();
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    app_widgets.mode_grid.set_callback(
                        move |index| {
                            let app_widgets = rc_app_widgets_clone_clone.borrow();
                            let mut r = radio_mutex_clone.radio.lock().unwrap();
                            let mut rx = 0;
                            ...
                            let (low, high) = app_widgets
                                .filter_grid
                                .get_filter_values(index, r.receiver[rx].filter);
                            app_widgets.filter_grid.set_active_values(low, high);
                            r.receiver[rx].filter_low = low;
                            r.receiver[rx].filter_high = high;
                    ...
                    let radio_mutex_clone = radio_mutex.clone();
                    let rc_app_widgets_clone_clone = rc_app_widgets_clone.clone();
                    app_widgets.filter_grid.set_callback(
                        move |index| {
                            let mut r = radio_mutex_clone.radio.lock().unwrap();
                            let mut rx = 0;
                            if r.receiver[1].active {
                            ...
                            let (low, high) = app_widgets
                                .filter_grid
                                .get_filter_values(r.receiver[rx].mode, r.receiver[rx].filter);
                            app_widgets.filter_grid.set_active_values(low, high);
                            r.receiver[rx].filter_low = low;
                            r.receiver[rx].filter_high = high;
                    ...
                    app_widgets
                        ...
                        .connect_clicked(move |button| {
                            ...
                            if r.mox && app_widgets.tun_button.is_active() {
                                ...
                                r.tune = false;
                                r.transmitter
                                    .set_tuning(r.tune, r.cw_keyer_sidetone_frequency);
                            }
                            r.updated = true;
                            r.set_state();
                            if r.mox {
                    ...
                    app_widgets
                        ...
                        .connect_value_changed(move |adjustment| {
                            ...
                            if let Ok(ref mut mutex) = lock {
                                ...
                                app_widgets
                                    .filter_grid
                                    .set_filter_low(adjustment.value() as f32);
                    ...
                    app_widgets
                        ...
                        .connect_value_changed(move |adjustment| {
                            ...
                            if let Ok(ref mut mutex) = lock {
                                ...
                                app_widgets
                                    .filter_grid
                                    .set_filter_high(adjustment.value() as f32);
                    ...
                    // initialize ui
                    {
                        ...
                        app_widgets
                            .filter_grid
                            .update_filter_buttons(r.receiver[rx].mode);
                        r.audio[0].init();
                        r.audio[1].init();
                        r.receiver[rx].set_mode();
                        r.transmitter.init();
                    ...
                    let rc_spectrum_clone2 = rc_spectrum_clone.clone();
                    let rc_spectrum_2_clone2 = rc_spectrum_2_clone.clone();
                    let spectrum_timeout_id = timeout_add_local(
                        Duration::from_millis(update_interval as u64),
                        move || {
                            let mut rx2 = false;
                            let r = radio_mutex_clone.radio.lock().unwrap();
                            rx2 = r.rx2_enabled;
                    ...
                    let rc_waterfall_clone2 = rc_waterfall_clone.clone();
                    let rc_waterfall_2_clone2 = rc_waterfall_2_clone.clone();
                    let waterfall_timeout_id = timeout_add_local(
                        Duration::from_millis(update_interval as u64),
                        move || {
                            let mut rx2 = false;
                            let r = radio_mutex_clone.radio.lock().unwrap();
                    ...
                    let rc_app_widgets_clone2 = rc_app_widgets_clone.clone();
                    let rc_meter_1_clone2 = rc_meter_1_clone.clone();
                    let meter_1_timeout_id = timeout_add_local(
                        Duration::from_millis(update_interval as u64),
                        move || {
                            meter_1_update(
                                &radio_mutex_clone,
                    ...
                    let rc_app_widgets_clone2 = rc_app_widgets_clone.clone();
                    let rc_meter_2_clone2 = rc_meter_2_clone.clone();
                    let meter_2_timeout_id = timeout_add_local(
                        Duration::from_millis(update_interval as u64),
                        move || {
                            meter_2_update(
                                &radio_mutex_clone,
...
fn spectrum_update(
    ...
    rc_app_widgets: &Rc<RefCell<AppWidgets>>,
    rc_spectrum: &Rc<RefCell<Spectrum>>,
) {
    let r = radio_mutex.radio.lock().unwrap();
...
fn waterfall_update(
    radio_mutex: &RadioMutex,
    rc_app_widgets: &Rc<RefCell<AppWidgets>>,
    rc_waterfall: &Rc<RefCell<Waterfall>>,
) {
    let r = radio_mutex.radio.lock().unwrap();
    let is_transmitting = r.is_transmitting();
    drop(r);
...
fn spectrum_waterfall_clicked(
    ...
    let display_frequency_offset =
        ((frequency_range - display_frequency_range) / 100.0) * r.receiver[rx].pan as f32;
